function [diff, src, dst] = func_extract_bias(mag, gyro, bias, idx, searchRange, featureRange, prc, status)
...
for cnt = 1:length(src.pts)
    if status
        ...
        % rotated = quatrotate(rotQ, src.mag(cnt, :));
        
        diff = [diff; dst.mag - rotated];
    else
        rotated = quatrotate(quatinv(dst.q(cnt, :)), dst.mag(cnt, :));
        rotated = quatrotate(src.q, rotated);
        
        diff = [diff; src.mag - rotated];
    end
end
