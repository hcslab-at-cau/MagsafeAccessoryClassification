...
params.eval.frontMargin = params.data.rate;
params.eval.frontMargin = 0;
m = zeros(length(params.global.all), length(params.global.all));
% m = zeros(length(params.global.all) + 1, length(params.global.all) + 1);
    ...
    for cnt2 = 1:cur.nEvent % Almostly 1
        for cnt3 = 1:2
            ...
            range = max(1, test.event(cnt2, cnt3) - params.eval.accMargin):...
                min(length(test.detect.all), test.event(cnt2, cnt3) + params.eval.frontMargin);
            
            % If any kind of event was detected
            if sum(test.detected(range) ~= 0) > 0
                % cur.isDetected(cnt2, cnt3) = true;
                id(range) = "";

                if cnt3 == 1 && sum(~strcmp(test.identify.id(range), "")) > 0
                    if sum(~strcmp(test.identify.id(range), "NIL")) > 0
                        idx = find(~strcmp(test.identify.id(range), "") & ~strcmp(test.identify.id(range), "NIL"));
                        cur.isDetected(cnt2, cnt3) = true;
                        biasResult(cnt).bias = test.identify.bias(range(idx), :);
                    end
                ...
                if (cnt3 == 1 && sum(test.identify.id(range) == test.name) > 0) || ...
                    (cnt3 == 2 && sum(test.identify.id(range) == "NIL") > 0)
                    if cnt3 == 1
                        idx = range(strcmp(test.identify.id(range), test.name));
                    else
                        idx = range(strcmp(test.identify.id(range), "NIL"));
                    end

                    % id(idx) = "";
                    cur.isIdentified(cnt2, cnt3) = true;
                    % if cnt3 == 1 && length(find(~strcmp(test.identify.id(range), ""))) == 1
                    %     cur.isIdentified(cnt2, cnt3) = true;
                    % elseif cnt3 == 2 
                    %     cur.isIdentified(cnt2, cnt3) = true;
                    % end
...
summary.acc = zeros(params.data.nObjects + 1, 1);
...
summary.acc = sum(summary.iAcc, 2) ./ (summary.nEvent*2);
summary.a = summary.dAcc;
summary.b = summary.iAcc;
...
summary.labels = {ori.name};

rowNames = {data(:).name};
rowNames = [rowNames, 'Average'];

% colNames = {'Detection (A)', 'Detection (D)', 'Identification', 'False Positive', 'FP(A)', 'FP(D)'};
colNames = {'D (A)', 'D (D)', 'Classification', 'Identification', 'False Positive', 'FP(A)', 'FP(D)'};
table = array2table([summary.dAcc(:, 1)*100, summary.iAcc(:, 2)*100, summary.iAcc(:, 1)*100, summary.acc*100,  mean(summary.fps, 2) * 100, summary.fps * 100], ...
